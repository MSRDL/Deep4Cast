...
def _compute_anomaly_score(coefs, values):
    b1 = b2 = 0
...
def _partition_anomalies(windows, k):
    ...
    return sorted(top_jump_positions, reverse=True)
...
def detect_anomalies(data, window_size, sensitivity=5, num_anomalies=None, visualize=True):
    ...
    w = window_size
    coefs = _compute_coefs(w)

    values = data.values
    num_windows = len(values) - w + 1
    windows = [(ix, _compute_anomaly_score(coefs, values[ix:ix + w])[0])
               for ix in range(num_windows)]
    ...
    if visualize:
        ...
        for anomaly in anomalies.values:
            plt.axvspan(anomaly[1], anomaly[2], color=plt.cm.jet(0.5 + float(anomaly[0]) / sensitivity / 2), alpha=0.5)
...
class StreamingAnomalyDetector:
    def __init__(self, windowSize, thresholds):
        ...
        self._coefs = _compute_coefs(windowSize)
    ...
    def score(self, value):
        ...
        return _compute_anomaly_score(self._coefs, self._buffer)
